...
def create_driver():
    """Создаёт и возвращает настроенный экземпляр драйвера."""
...
def get_all_offer_data_from_config(driver):
    """Извлекает все данные об объявлении из window._cianConfig"""
    try:
        script = """
        try {
            ...
            if (configs && configs.length > 0) {
                for (var i = 0; i < configs.length; i++) {
                    if (configs[i].key === 'defaultState' && configs[i].value) {
                        return JSON.stringify(configs[i].value);
                    }
                }
            }
            return null;
        ...
        if floor and all_floor:
            floor_info = f"{floor} из {all_floor}"
        else:
            floor_info = f"{floor}" if floor else "Не указан"
...
def run_parser(pages: int = 1) -> int:
    """
    Запускает полный цикл парсинга Циан.
    Принимает: pages — количество страниц поисковой выдачи.
    Возвращает: количество успешно собранных объявлений.
    """
    driver = create_driver()
    all_links = []  # список ссылок (без JSON-файла)
    all_data = []   # список словарей с данными
    ...
    try:
        ...
        for page in range(1, pages + 1):
            ...
            cards = driver.find_elements(By.CSS_SELECTOR, '[data-testid="offer-card"]')
            for card in cards:
                try:
                    title = card.find_element(By.CSS_SELECTOR, '[data-mark="OfferTitle"] span').text
                    link_elem = card.find_element(By.CSS_SELECTOR, '[data-name="LinkArea"] a')
                    link_url = link_elem.get_attribute('href')
                    all_links.append({'title': title, 'url': link_url, 'page': page})
        ...
        for idx, link_info in enumerate(all_links, 1):
            ...
            try:
                data = get_all_offer_data_from_config(driver)
                if not data:
                    print("  ❌ Данные не извлечены")
                    continue
                titlename = driver.find_element(By.CSS_SELECTOR, '[data-name="OfferTitleNew"] h1').text
    ...
    end_time = time.time()  # ← ЗАМЕР КОНЦА
